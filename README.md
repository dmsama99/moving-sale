...
- 卡片背景那层反正要模糊 16px，只取 96 宽——单张从 134 KB 降到 1.1 KB。
  这层是 `loading="eager"`，31 张一进页面就全下，所以这一档最要紧：**总共才 33 KB**。
- **这四档一个都不能缺**，`cdn()` 这四个宽度都会调用，缺哪档哪档图裂。

实测（31 张原图 → 124 个 webp）：

| 档 | 合计 | 平均 | 用在哪 |
|---|---|---|---|
| w96 | 33 KB | 1.1 KB | 模糊背景层，`eager` |
| w400 | 468 KB | 15.1 KB | 手机窄屏 |
| w800 | 1.33 MB | 44.1 KB | 默认档，比原图省 67% |
| w1200 | 2.63 MB | 86.7 KB | 大屏 / 高 DPI |
| 原图 | 4.06 MB | 134 KB | 只在 webp 出问题时兜底 |

原图长边统一 1200 px：23 张横的 1200×900，8 张竖的 900×1200。
`build-images.sh` 逐张比对宽度决定缩不缩，**不会把小图放大**。
