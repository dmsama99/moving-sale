...
这个分支（`cloudflare`）做完了所有代码改动。**但还差一步必须你自己跑**：生成 webp 图片。
没跑之前别部署，原因见下面「为什么图片这步不能跳」。
...
─────────────────────────          ─────────────────────────
装 cwebp                            建 Pages 项目
跑 build-images.sh                  临时域名自测
push 到 Netlify 验证  ←── 安全点     删旧 DNS → 绑自定义域
                                    过一遍开关清单 → 验收
```
...
现在靠 Netlify 现场压到 96 宽，每张约 1.7 KB，31 张加起来才 50 KB 左右。
一旦图片接口没了、走了第 601 行的兜底，这层会退回原图：

```
photos/ 实测：31 张 900×1200，共 4,257,440 字节（4.06 MB），平均 137 KB/张
```

**首屏会从 ~50 KB 变成 4.2 MB，涨 80 倍，而且是并发的。** 国内手机网络下基本等于打不开。
所以「先搬上去再说，图片慢慢优化」这条路走不通——图片必须先解决。

---

## 第一段 · 在 Netlify 上做完（约 15 分钟）
...
（本机确认过：没有 cwebp、没有 ImageMagick、没有 node。macOS 自带的 `sips` 能读 webp
但**不能写**，实测报 `Error: Can't write format: org.webmproject.webp`，所以只能装。）

### 2. 跑构建脚本

```bash
cd /Users/kai/Desktop/moving-sale/site
./build-images.sh
```

跑完会打印每档的张数和体积，四档张数必须都等于原图张数（现在是 31），对不上它会警告。
预期产物：

```
photos/w96/    31 个   ~50 KB     模糊背景层用，反正要糊 16px，压到 q45
photos/w400/   31 个   ~0.4 MB    手机窄屏
photos/w800/   31 个   ~1.2-1.7 MB  默认档
photos/w1200/  31 个   ~2.0-2.5 MB  大屏 / 高 DPI
```

仓库总体积会到 8.5 MB 左右。GitHub 完全无压力（单文件上限 100 MB，仓库软上限 1 GB）。

> 脚本会跳过原图没变过的档，所以以后重复跑很快。想全部重压：`./build-images.sh --force`
...
- **TXT 记录**（各类所有权验证、SPF）
- **CAA 记录**：有的话确认允许 Cloudflare 签证书，否则第 7 步的自定义域证书会失败。没有就不用管
...
3. **`*.pages.dev` 上用不了**（官方错误码 9524 明确写 *"use a Custom Domain"*）
   → 所以第 6 步用临时域名自测时图片一定失败，**这是预期现象**，必须绑完自定义域才能测
